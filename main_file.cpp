...
void keyCallback(GLFWwindow* window,int key,int scancode,int action,int mods) {
	...
	if (action==GLFW_PRESS) {
		...
		if (key == GLFW_KEY_UP) robotspeedx = PI ;
...
void initOpenGLProgram(GLFWwindow* window) {
	...
	tex[1] = readTexture("StoneBricksBeige015_DISP_2K.png");
	tex[2] = readTexture("StoneMarbleCalacatta004_COL_2K.png");
...
void drawScene(GLFWwindow* window,float angle_x,float angle_y, float glowaangle, float anglex, float angley) {
...
int main(void)
{
	...
	while (!glfwWindowShouldClose(window)) //Tak długo jak okno nie powinno zostać zamknięte
	{
        ...
        glfwSetTime(0); //Zeruj timer
		drawScene(window,angle_x,angle_y, glowaangle, anglex, angley); //Wykonaj procedurę rysującą
